...
<div class="space-y-5">
  ...
  <div
    ...
    <div class="flex items-start gap-3">
      ...
      <div>
        ...
        <p class=" text-xs 2xl:text-sm text-slate-500">Manage welfare funds, assistance requests, approvals and
          disbursements.</p>
      </div>
    </div>
  ...
  <div class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
    ...
    ['submitted','Pending','bi-hourglass-split','border-amber-200 bg-amber-50/40','text-amber-700','bg-amber-100
    text-amber-600'],
    ['approved','Approved','bi-patch-check','border-indigo-200 bg-indigo-50/40','text-indigo-700','bg-indigo-100
    text-indigo-600'],
    ['completed','Completed','bi-check2-circle','border-emerald-200 bg-emerald-50/40','text-emerald-700','bg-emerald-100
    text-emerald-600'],
    ['total_disbursed','Disbursed','bi-cash-stack','border-rose-200 bg-rose-50/40','text-rose-700','bg-rose-100
    text-rose-600'],
    ];
    @endphp
      ...
      <div class="grid w-full gap-2 sm:grid-cols-2 lg:w-auto lg:grid-cols-[290px_180px_auto] lg:gap-0">
        <div class="relative sm:col-span-2 lg:col-span-1">
          ...
          <input id="searchInput" type="text" placeholder="Search welfare requests..."
            class="h-9 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3  text-xs 2xl:text-sm text-slate-700 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 lg:rounded-r-none">
        </div>

        <select id="statusFilter"
          class="h-9 rounded-md border border-slate-300 bg-white px-3  text-xs 2xl:text-sm font-medium text-slate-600 outline-none focus:border-indigo-400 lg:rounded-none lg:border-l-0">
        ...
        <button type="button" onclick="clearFilters()"
          class="inline-flex h-9 items-center justify-center gap-1.5 rounded-md border border-slate-300 bg-slate-50 px-3  text-xs 2xl:text-sm font-semibold text-slate-600 hover:bg-slate-100 lg:rounded-l-none lg:border-l-0">
  ...
  <div id="fundModal" class="app-modal-overlay fixed inset-0 z-50 hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-xl overflow-hidden rounded-md bg-white">
      <div class="flex items-start justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('fundModal')" class="app-modal-close"><i
        ...
        <div class="space-y-4 p-5">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input id="fundName" type="text" maxlength="150" class="app-input w-full"
                placeholder="General Welfare Fund">
  ...
  <div id="allocationModal" class="app-modal-overlay fixed inset-0 z-50 hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-xl overflow-hidden rounded-md bg-white">
      <div class="flex items-start justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('allocationModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>
  ...
  <div id="requestModal" class="app-modal-overlay fixed inset-0 z-50 hidden items-center justify-center p-3 sm:p-5">
    <div class="app-modal-panel flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-md bg-white">
      <div class="flex items-start justify-between border-b border-slate-200 px-5 py-4">
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 items-center justify-center rounded-md bg-rose-50 text-rose-600"><i
        ...
        <button type="button" onclick="AdminUI.closeModal('requestModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>
        ...
        <div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <select id="requestMemberId" class="app-input w-full">
            ...
            <div>
              ...
              <select id="welfareFundId" class="app-input w-full">
          ...
          <div>
            ...
            <textarea id="requestReason" rows="4" maxlength="5000" class="app-input w-full resize-none"
              placeholder="Describe the reason for requesting assistance..."></textarea>
  ...
  <div id="manageModal" class="app-modal-overlay fixed inset-0 z-50 hidden items-center justify-center p-3 sm:p-5">
    <div class="app-modal-panel flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-md bg-white">
      ...
      <div class="flex items-start justify-between border-b border-slate-200 px-5 py-4">
        <div class="flex items-center gap-3">
          <div class="flex h-10 w-10 items-center justify-center rounded-md bg-indigo-50 text-indigo-600"><i
              class="bi bi-heart-pulse"></i></div>
          <div>
  ...
  <div id="reviewModal" class="app-modal-overlay fixed inset-0 z-[60] hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-lg rounded-md bg-white">
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('reviewModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>
      ...
      <form id="reviewForm" novalidate data-js-validation="1">
        ...
        <div class="flex justify-end gap-2 border-t border-slate-200 px-5 py-4">
          <button type="button" onclick="AdminUI.closeModal('reviewModal')"
            class="inline-flex items-center justify-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            <i class="bi bi-x-lg"></i> Cancel
          </button>
          <button id="reviewButton" type="submit"
            class="inline-flex items-center justify-center gap-1.5 rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">
  ...
  <div id="approveModal" class="app-modal-overlay fixed inset-0 z-[60] hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-lg rounded-md bg-white">
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('approveModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>
      ...
      <form id="approveForm" novalidate data-js-validation="1">
        ...
        <div class="flex justify-end gap-2 border-t border-slate-200 px-5 py-4">
          <button type="button" onclick="AdminUI.closeModal('approveModal')"
            class="inline-flex items-center justify-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            <i class="bi bi-x-lg"></i> Cancel
          </button>
  ...
  <div id="rejectModal" class="app-modal-overlay fixed inset-0 z-[60] hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-lg rounded-md bg-white">
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('rejectModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>
  ...
  <div id="disburseModal" class="app-modal-overlay fixed inset-0 z-[60] hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-lg rounded-md bg-white">
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('disburseModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>

      <form id="disburseForm" novalidate data-js-validation="1">
        <div class="space-y-4 p-5">
          <div id="disburseError"
            class="hidden rounded-md border border-red-200 bg-red-50 px-3 py-2 text-base text-red-700"></div>
        ...
        <div class="flex justify-end gap-2 border-t border-slate-200 px-5 py-4">
          <button type="button" onclick="AdminUI.closeModal('disburseModal')"
            class="inline-flex items-center justify-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            <i class="bi bi-x-lg"></i> Cancel
          </button>
  ...
  <div id="reverseModal" class="app-modal-overlay fixed inset-0 z-[60] hidden items-center justify-center p-3">
    <div class="app-modal-panel w-full max-w-lg rounded-md bg-white">
      <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        ...
        <button type="button" onclick="AdminUI.closeModal('reverseModal')" class="app-modal-close"><i
            class="bi bi-x-lg"></i></button>
      </div>
      ...
      <form id="reverseForm" novalidate data-js-validation="1">
        ...
        <div class="flex justify-end gap-2 border-t border-slate-200 px-5 py-4">
          <button type="button" onclick="AdminUI.closeModal('reverseModal')"
            class="inline-flex items-center justify-center gap-1.5 rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            <i class="bi bi-x-lg"></i> Cancel
          </button>
          <button id="reverseButton" type="submit"
            class="inline-flex items-center justify-center gap-1.5 rounded-md bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">
...
$('requestMobileGrid').innerHTML=`<div class="px-4 py-10 text-center text-sm text-red-600">${esc(message)}</div>`;
}
}
...
<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-rose-500">
<i class="bi bi-heart-pulse"></i>
</div>
<div class="min-w-0">
...
<p class="truncate text-xs 2xl:text-sm font-semibold text-slate-700">${esc(r.member?.user?.name??'N/A')}</p>
<p class="truncate text-[11px] text-indigo-600">${esc(r.member?.member_code??'')}</p>
</div>
...
<button type="button" onclick="openManageModal(${r.id})" class="flex h-8 flex-1 items-center justify-center gap-1.5 rounded-md bg-indigo-50 px-2 text-[11px] font-semibold text-indigo-600 transition hover:bg-indigo-100">
<i class="bi bi-eye text-sm"></i>
Manage
</button>
</div>
...
return`<button type="button" onclick="${handler}" class="inline-flex items-center gap-1.5 rounded-md border px-3 py-2 text-xs 2xl:text-sm font-semibold transition ${styles[color]}"><i class="bi ${icon}"></i>${label}</button>`;
}

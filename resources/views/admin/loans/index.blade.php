...
<p class="text-sm text-slate-500">Manage member loan requests, approvals, disbursements and repayments.</p>
</div>
</div>
...
<input id="searchInput" type="text" placeholder="Search loans..." class="h-9 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-700 outline-none placeholder:text-slate-400 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 lg:rounded-r-none">
</div>
...
<tr>
<td colspan="8" class="px-4 py-10 text-center text-sm text-slate-400">Loading loans...</td>
</tr>
</tbody>
</table>
</div>
...
<div>
<h3 class="text-lg font-bold text-slate-800">Create Loan Request</h3>
...
<p data-field-error="requestedAmount" class="mt-1 hidden text-xs text-red-600"></p>
</div>
...
<p data-field-error="requestDate" class="mt-1 hidden text-xs text-red-600"></p>
</div>
</div>
...
<div>
<h3 id="manageLoanTitle" class="text-lg font-bold text-slate-800">Loan Details</h3>
...
<div id="manageLoanError" class="hidden rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700"></div>
...
<div id="loanPurpose" class="whitespace-pre-line rounded-md bg-slate-50 p-3 text-xs leading-5 text-slate-600"></div>
...
<p id="loanRejectionReason" class="mt-1 whitespace-pre-line text-xs text-red-700"></p>
</div>
</section>
</div>
...
<p data-field-error="interestRate" class="mt-1 hidden text-xs text-red-600"></p>
</div>
...
<p data-field-error="durationMonths" class="mt-1 hidden text-xs text-red-600"></p>
</div>
</div>
...
<button id="approveButton" type="submit" class="rounded-md bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-60">Approve Loan</button>
</div>
</form>
</div>
</div>
...
<button id="rejectLoanButton" type="submit" class="rounded-md bg-red-600 px-4 py-2 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-60">Reject Loan</button>
</div>
</form>
</div>
</div>
...
<p data-field-error="disbursementAccountId" class="mt-1 hidden text-xs text-red-600"></p>
</div>
...
<p data-field-error="receiveAccountId" class="mt-1 hidden text-xs text-red-600"></p>
</div>
...
<p data-field-error="repaymentAmount" class="mt-1 hidden text-xs text-red-600"></p>
</div>
...
<p data-field-error="repaymentDate" class="mt-1 hidden text-xs text-red-600"></p>
</div>
</div>
...
<button id="repayButton" type="submit" class="rounded-md bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-700 disabled:opacity-60">Receive Repayment</button>
</div>
</form>
</div>
</div>
...
function renderLoans(){
    if(!loans.length){
        ...
        $('loanMobileGrid').innerHTML=`
            <div class="col-span-full rounded-md border border-slate-200 bg-white p-8 text-center">
                ...
                <p class="mt-3 text-sm font-semibold text-slate-600">No loans found</p>
                <p class="mt-1 text-xs text-slate-400">Try changing the search or status filter.</p>
            </div>
        `;
        return;
    }
    ...
    $('loanTableBody').innerHTML=loans.map(loan=>`
        <tr class="transition hover:bg-slate-50/70">
            <td class="px-4 py-3">
                <div class="font-semibold text-slate-700">${esc(loan.loan_no)}</div>
                <div class="mt-0.5 text-[11px] text-slate-400">${date(loan.request_date)}</div>
            </td>

            <td class="px-4 py-3">
                <div class="font-semibold text-slate-700">${esc(loan.member?.user?.name??'N/A')}</div>
    ...
    $('loanMobileGrid').innerHTML=loans.map(loan=>`
        <article class="overflow-hidden rounded-md border border-slate-200 bg-white">
            ...
            <div class="border-b border-slate-100 px-4 py-3">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-sm font-bold text-slate-700">${esc(loan.loan_no)}</p>
            ...
            <div class="space-y-3 p-4">
                <div>
                    <p class="text-xs font-semibold text-slate-700">${esc(loan.member?.user?.name??'N/A')}</p>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="rounded-md bg-slate-50 p-3">
                        ...
                        <p class="mt-1 text-sm font-bold text-slate-700">${money(loan.requested_amount)}</p>
                    </div>

                    <div class="rounded-md bg-indigo-50 p-3">
                        <p class="text-[10px] uppercase text-indigo-400">Approved</p>
                        <p class="mt-1 text-sm font-bold text-indigo-700">${loan.approved_amount?money(loan.approved_amount):'—'}</p>
                    </div>
                </div>
            ...
            <div class="border-t border-slate-100 bg-slate-50/50 px-4 py-3">
                <button type="button" onclick="openManageLoan(${loan.id})"
                    class="inline-flex w-full items-center justify-center gap-1 rounded-md border border-indigo-200 bg-white px-3 py-2 text-xs font-semibold text-indigo-700">
                    <i class="bi bi-eye"></i>
                    Manage Loan
                </button>
            </div>
...
function renderRepayments(repayments){
    ...
    $('repaymentHistory').innerHTML=repayments.map(payment=>`
        <div class="flex items-center justify-between gap-3 rounded-md border border-slate-200 bg-slate-50/60 px-3 py-2.5">
            <div>
                <p class="text-xs font-semibold text-slate-700">${money(payment.total_amount??payment.amount)}</p>
    ...
    $('approvalPreview').innerHTML=`
        <div class="grid grid-cols-3 gap-2">
            <div>
                ...
                <p class="mt-1 text-xs font-bold text-indigo-700">${money(principal)}</p>
            </div>
            <div>
                <p class="text-[10px] uppercase text-indigo-400">Interest</p>
                <p class="mt-1 text-xs font-bold text-indigo-700">${money(interest)}</p>
            </div>
            <div>
                <p class="text-[10px] uppercase text-indigo-400">Total</p>
                <p class="mt-1 text-xs font-bold text-indigo-700">${money(total)}</p>
            </div>
        </div>

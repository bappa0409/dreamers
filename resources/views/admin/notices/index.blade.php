...
<div class="space-y-5">
    <div class="flex flex-col gap-4 rounded-md border border-slate-200 bg-white p-5 shadow-sm md:flex-row md:items-center md:justify-between">
        <div class="flex items-start gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-slate-800">Notice Management</h1>
                <p class="mt-1 text-sm text-slate-500">Create, schedule, publish and notify members.</p>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[1000px] text-sm">
                <thead class="border-b border-slate-200 bg-slate-50">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600">Notice</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600">Type</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600">Priority</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600">Publish</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600">Creator</th>
    ...
    <div class="app-modal-panel flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-md bg-white">
        <div class="app-modal-header flex shrink-0 items-start justify-between border-b border-slate-200 px-5 py-5">
            <div>
                ...
                <p class="mt-1 text-xs text-slate-500">Create and publish association notice.</p>
            </div>
        ...
        <form id="noticeForm" class="flex min-h-0 flex-1 flex-col">
            <div class="space-y-4 overflow-y-auto p-5">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="publishAt" type="text" class="app-input js-datetime-picker" placeholder="YYYY-MM-DD HH:MM" autocomplete="off">
                    </div>

                    <div>
                        <label class="form-label">Expires At</label>
                        <input id="expiresAt" type="text" class="app-input js-datetime-picker" placeholder="YYYY-MM-DD HH:MM" autocomplete="off">
                    </div>
                </div>
                ...
                @if(auth()->user()->hasPermission('Notification.send'))
                    <label class="flex cursor-pointer items-center gap-3 rounded-md border border-indigo-100 bg-indigo-50/40 p-3">
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-[11px] text-indigo-500">Send a notification when this notice is published now.</p>
                        </div>
                    </label>
                    ...
                    <div id="audienceSection" class="hidden rounded-md border border-slate-200 bg-slate-50/40 p-4">
                        ...
                        <div id="userAudience" class="mt-3 hidden">
                            ...
                            <div id="userResults" class="mt-2 max-h-44 overflow-y-auto rounded-md border border-slate-200 bg-white p-2"></div>
                        </div>
                    </div>
                @endif
            ...
            <div class="flex shrink-0 justify-end gap-2 border-t border-slate-200 px-5 py-4">
                <button type="button" onclick="closeNoticeModal()" class="cursor-pointer rounded-md border border-slate-300 px-4 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50">
                    Cancel
                </button>

                <button id="saveButton" type="submit" class="cursor-pointer rounded-md bg-indigo-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-indigo-700">
                    Save Notice
                </button>
            </div>
        </form>
...
async function loadNotices(page=1){
    ...
    try{
        ...
        notices=paginator.data??[];
        currentPage=paginator.current_page??1;
        lastPage=paginator.last_page??1;
        total=paginator.total??notices.length;
        ...
        <tr class="border-b border-slate-100 transition hover:bg-slate-50">
            <td class="px-4 py-4">
                <div class="max-w-[360px]">
                    ...
                    ${item.attachment?`
                        <div class="mt-1.5 inline-flex items-center gap-1 text-[10px] text-indigo-500">
                            <i class="bi bi-paperclip"></i>
                            Attachment
                        </div>
                    `:''}
            ...
            <td class="px-4 py-4">
                <span class="rounded-md bg-sky-50 px-2 py-1 text-[10px] font-semibold capitalize text-sky-700">
                    ${AdminUI.escapeHtml(item.type)}
                </span>
            </td>
...
function priorityBadge(priority){
    ...
    return `
        <span class="rounded-md px-2 py-1 text-[10px] font-semibold capitalize ${map[priority]??map.normal}">
            ${AdminUI.escapeHtml(priority)}
        </span>
    `;
}
...
function publishBadge(item){
    if(!item.is_published){
        return '<span class="rounded-md bg-slate-100 px-2 py-1 text-[10px] font-semibold text-slate-500">Draft</span>';
    }

    if(item.publish_at&&new Date(item.publish_at)>new Date()){
        return '<span class="rounded-md bg-indigo-50 px-2 py-1 text-[10px] font-semibold text-indigo-600">Scheduled</span>';
    }

    if(item.expires_at&&new Date(item.expires_at)<new Date()){
        return '<span class="rounded-md bg-red-50 px-2 py-1 text-[10px] font-semibold text-red-600">Expired</span>';
    }
...
function openNoticeModal(notice=null){
    ...
    if(notice){
        ...
        if(notice.attachment){
            el.attachmentName.innerText='Existing attachment';
        }
    }else{
    ...
    if(canNotify){
        ...
        const audience=document.getElementById('audienceSection');
...
el.form.addEventListener('submit',async event=>{
    ...
    formData.append(
        'is_published',
        el.isPublished.checked?'1':'0'
    );

    if(el.publishAt.value){
        formData.append(
            'publish_at',
            el.publishAt.value
        );
    ...
    if(el.attachment.files[0]){
        formData.append(
            'attachment',
            el.attachment.files[0]
        );
    }

    if(canNotify&&!editingNotice){
        const notify=
            document.getElementById('notifyMembers').checked;
...
function togglePublish(id){
    AdminUI.request(
        `/api/notices/${id}/toggle-publish`,
        {
            method:'PATCH',
            data:{},
            confirmMessage:'Change publish status?',
            successMessage:'Publish status updated.',
            onSuccess:async()=>{
...
async function loadRecipients(search=''){
    ...
    try{
        ...
        const response=await api(
            `/api/notification-campaigns/recipients?${query}`
        );
...
function renderUsers(users){
    ...
    if(!users.length){
        container.innerHTML=`
            <p class="p-3 text-center text-xs text-slate-400">
                No users found.
            </p>
        `;
    ...
    container.innerHTML=users.map(user=>`
        <label class="flex cursor-pointer items-center gap-2 rounded-md p-2 transition hover:bg-slate-50">
            <input type="checkbox" value="${user.id}" ${selectedUsers.has(Number(user.id))?'checked':''} onchange="toggleUser(${user.id},this.checked)" class="h-4 w-4 rounded border-slate-300 text-indigo-600">
...
async function initNoticePage(){
    ...
    el.priorityFilter.addEventListener(
        'change',
        ()=>loadNotices(1)
    );

    if(canNotify){
        document.getElementById('notifyMembers')
            .addEventListener('change',function(){
                document.getElementById('audienceSection')
                    .classList.toggle(
                        'hidden',
                        !this.checked
                    );
        ...
        document.getElementById('audienceType')
            .addEventListener('change',function(){
                document.getElementById('roleAudience')
                    .classList.toggle(
                        'hidden',
                        this.value!=='role'
                    );

                document.getElementById('userAudience')
                    .classList.toggle(
                        'hidden',
                        this.value!=='users'
                    );

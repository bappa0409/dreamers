...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-4 rounded-lg border border-slate-200 bg-white p-5 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-4">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-sm">
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-slate-500">
                    View published announcements, events and important association notices.
                </p>
            </div>
        </div>

        <button type="button" onclick="loadNotices(1)" class="inline-flex h-9 w-fit items-center justify-center gap-2 rounded-md border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-600 transition hover:border-amber-200 hover:bg-amber-50 hover:text-amber-700">
    ...
    <div class="rounded-lg border border-slate-200 bg-white p-4">
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-12 lg:items-end">
            ...
            <div class="lg:col-span-5">
                ...
                <div class="relative">
                    ...
                    <input
                        ...
                        class="h-9 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-amber-400 focus:ring-2 focus:ring-amber-100">
                </div>
            </div>
            ...
            <div class="lg:col-span-3">
                ...
                <select
                    id="typeFilter"
                    class="h-9 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-700 outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-100">
    ...
    <div id="noticeContainer" class="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div class="col-span-full rounded-lg border border-slate-200 bg-white py-14 text-center">
            <div class="mx-auto h-7 w-7 animate-spin rounded-full border-4 border-slate-200 border-t-amber-500"></div>
            <p class="mt-3 text-xs text-slate-400">Loading notices...</p>
        </div>
    </div>
...
<div id="noticeModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 p-4 backdrop-blur-[1px]">
    <div class="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl">
        ...
        <div id="noticeModalBody" class="min-h-0 flex-1 overflow-y-auto p-5"></div>
...
async function loadNotices(page=1){
    ...
    try{
        ...
        const response=await api(`/api/member/notices?${params.toString()}`);
        const paginator=response.data??{};

        notices=Array.isArray(paginator.data)
            ?paginator.data
            :[];
        ...
        container.innerHTML=`
            <div class="col-span-full rounded-lg border border-red-200 bg-red-50/40 px-5 py-14 text-center">
...
function renderNotices(){
    ...
    if(!notices.length){
        container.innerHTML=`
            <div class="col-span-full rounded-lg border border-slate-200 bg-white px-4 py-16 text-center">
    ...
    container.innerHTML=notices.map(item=>{
        ...
        return`
            <article
                ...
                class="group relative flex flex-col overflow-hidden rounded-lg border border-slate-200 bg-white transition duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md">
                ...
                <div class="flex-1 p-5">
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div class="min-w-0 flex-1">
                            ...
                            <h3 class="text-sm font-bold leading-6 text-slate-800">
                                ${escapeValue(item.title??'Untitled Notice')}
                            </h3>
                        </div>
                    ...
                    <p class="mt-3 line-clamp-3 whitespace-pre-line text-xs leading-6 text-slate-500">
                        ${escapeValue(body)}
                    </p>
                ...
                <div class="flex items-center justify-between gap-3 border-t border-slate-100 bg-slate-50/50 px-5 py-3">
                            ...
                            item.creator?.name
                                ?`
                                    <p class="mt-1 flex items-center gap-1.5 truncate text-[10px] text-slate-400">
                                        ...
                                        By ${escapeValue(item.creator.name)}
                                    </p>
                                `
                                :''
                        }
                    ...
                    <button
                        ...
                        class="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-md border border-slate-200 bg-white px-3 text-[11px] font-semibold text-slate-600 transition hover:border-amber-200 hover:bg-amber-50 hover:text-amber-700">
                        Read Notice
...
window.openNoticeModal=function(id){
    const item=notices.find(
        notice=>Number(notice.id)===Number(id)
    );
    ...
    const attachmentUrl=item.attachment
        ?`/storage/${String(item.attachment).replace(/^\/+/,'')}`
        :null;
        ...
        <div class="space-y-5">
            ...
            <div class="rounded-lg border border-slate-200 bg-white p-5">
                <div class="whitespace-pre-line break-words text-sm leading-7 text-slate-600">
                    ${escapeValue(stripHtml(item.content??''))}
                </div>
            </div>
                                ...
                                <div class="flex min-w-0 items-center gap-3">
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <p class="mt-0.5 truncate text-[10px] text-slate-400">
                                            ${escapeValue(fileName(item.attachment))}
                                        </p>
                                    </div>
                                </div>
...
function typeBadge(type){
    const styles={
        notice:'border-slate-200 bg-slate-100 text-slate-600',
        announcement:'border-indigo-200 bg-indigo-50 text-indigo-700',
        event:'border-sky-200 bg-sky-50 text-sky-700',
        urgent:'border-red-200 bg-red-50 text-red-700'
    };

    return`
        <span class="inline-flex rounded-md border px-2 py-1 text-[10px] font-semibold ${styles[type]??styles.notice}">
            ${escapeValue(titleCase(type??'notice'))}
        </span>
    `;
}
...
function priorityBadge(priority){
    ...
    return`
        <span class="inline-flex rounded-md border px-2 py-1 text-[10px] font-semibold ${styles[priority]??styles.normal}">
            ${escapeValue(titleCase(priority??'normal'))}
        </span>
    `;
}
...
function modalHeroStyle(priority){
    const styles={
        low:'border border-slate-200 bg-slate-50',
        normal:'border border-indigo-100 bg-indigo-50/40',
        high:'border border-amber-200 bg-amber-50/50',

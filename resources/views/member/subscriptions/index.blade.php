...
<div class="space-y-5">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-slate-500">
                    Track dues, payments, fines and monthly subscription history.
                </p>
            </div>
        </div>
    ...
    {{-- Summary --}}
    <div id="summaryCards" class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 {{ $shareEnabled?'2xl:grid-cols-6':'2xl:grid-cols-5' }}">
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-4">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p id="monthlyPayable" class="mt-2 text-xl font-bold text-slate-800">
                        {{ $currency }}0.00
                    </p>
                </div>
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-4">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p id="currentDueAmount" class="mt-2 text-xl font-bold text-slate-800">
                        {{ $currency }}0.00
                    </p>
                </div>
        ...
        <div class="rounded-lg border border-emerald-200 bg-emerald-50/40 p-4">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p id="currentPaidAmount" class="mt-2 text-xl font-bold text-emerald-700">
                        {{ $currency }}0.00
                    </p>
                </div>
        ...
        <div class="rounded-lg border border-amber-200 bg-amber-50/40 p-4">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p id="currentFineAmount" class="mt-2 text-xl font-bold text-amber-700">
                        {{ $currency }}0.00
                    </p>
                </div>
        ...
        <div class="rounded-lg border border-red-200 bg-red-50/40 p-4">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-[10px] font-medium uppercase tracking-wide text-red-500">
                        Outstanding
                    </p>

                    <p id="currentOutstandingAmount" class="mt-2 text-xl font-bold text-red-600">
                        {{ $currency }}0.00
                    </p>
                </div>
            ...
            <div class="rounded-lg border border-violet-200 bg-violet-50/40 p-4">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p id="activeShareCount" class="mt-2 text-xl font-bold text-violet-700">
                            0
                        </p>
                    </div>
            ...
            <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p id="currentPeriod" class="text-[11px] text-slate-400">
                            -
                        </p>
                    </div>
                </div>

                <span id="currentStatus" class="w-fit rounded-md bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600">
                    -
                </span>
            </div>
            ...
            <div id="currentDueContent" class="hidden">
                    ...
                    <div class="rounded-lg border border-slate-200 bg-slate-50/60 p-4">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p id="progressText" class="mt-0.5 text-[11px] text-slate-400">
                                    {{ $currency }}0.00 paid
                                </p>
                            </div>

                            <span id="progressPercentage" class="text-xl font-bold text-indigo-600">
                                0%
                            </span>
                        </div>
                ...
                <div class="grid grid-cols-2 gap-3 p-5 md:grid-cols-5">
                    ...
                    <div class="rounded-lg border border-slate-200 p-4">
                        ...
                        <p id="baseAmount" class="mt-1.5 text-sm font-bold text-slate-800">
                            {{ $currency }}0.00
                        </p>
                    </div>

                    <div class="rounded-lg border border-violet-200 bg-violet-50/30 p-4">
                        <p class="text-[10px] uppercase tracking-wide text-violet-500">
                            Shares
                        </p>

                        <p id="shareCount" class="mt-1.5 text-sm font-bold text-violet-700">
                            1
                        </p>
                    </div>
                    ...
                    <div class="rounded-lg border border-amber-200 bg-amber-50/30 p-4">
                        ...
                        <p id="fineAmount" class="mt-1.5 text-sm font-bold text-amber-700">
                            {{ $currency }}0.00
                        </p>
                    </div>

                    <div class="rounded-lg border border-emerald-200 bg-emerald-50/30 p-4">
                        <p class="text-[10px] uppercase tracking-wide text-emerald-500">
                            Paid
                        </p>

                        <p id="paidAmount" class="mt-1.5 text-sm font-bold text-emerald-700">
                            {{ $currency }}0.00
                        </p>
                    </div>

                    <div class="col-span-2 rounded-lg border border-red-200 bg-red-50/30 p-4 md:col-span-1">
                        <p class="text-[10px] uppercase tracking-wide text-red-500">
                            Outstanding
                        </p>

                        <p id="outstandingAmount" class="mt-1.5 text-sm font-bold text-red-600">
                            {{ $currency }}0.00
                        </p>
                    </div>
                ...
                <div class="flex flex-col gap-4 border-t border-slate-200 bg-slate-50/30 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p id="dueDate" class="mt-0.5 text-xs font-semibold text-slate-700">
                                -
                            </p>
                        </div>
                    </div>
        ...
        <div class="overflow-hidden rounded-lg border border-slate-200 bg-white">
            ...
            <div class="p-5">
                ...
                <div class="rounded-lg bg-gradient-to-br from-indigo-600 to-violet-600 p-5 text-white">
                    <p class="text-[10px] font-medium uppercase tracking-[0.14em] text-indigo-100">
                        Active Plan
                    </p>

                    <h3 id="planName" class="mt-2 text-lg font-bold">
                        -
                    </h3>

                    <div class="mt-4 flex items-end gap-1">
                        <span id="planAmount" class="text-2xl font-bold">
                            {{ $currency }}0.00
                        </span>

                        <span class="pb-1 text-xs text-indigo-100">
                            / base month
                        </span>
                    </div>
                </div>
                ...
                <div class="mt-4 divide-y divide-slate-100 rounded-lg border border-slate-200">
                    <div class="flex items-center justify-between gap-3 px-4 py-3">
                        ...
                        <span id="planDueDay" class="text-xs font-semibold text-slate-700">
                            -
                        </span>
                    </div>
                    ...
                    <div class="flex items-center justify-between gap-3 px-4 py-3">
                        ...
                        <span id="subscriptionStartDate" class="text-xs font-semibold text-slate-700">
                            -
                        </span>
                    </div>
                    ...
                    @if($shareEnabled)
                        <div class="flex items-center justify-between gap-3 px-4 py-3">
                            ...
                            <span class="text-right text-[11px] font-semibold text-violet-600">
                                Base × Active Shares
                            </span>
                        </div>
                    @endif
        ...
        <div class="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-[11px] text-slate-400">
                        Monthly dues, fine and payment status
                    </p>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[980px]">
                ...
                <tbody id="subscriptionHistory" class="divide-y divide-slate-100">
                    <tr>
                        <td colspan="10" class="px-4 py-12 text-center text-xs text-slate-400">
    ...
    <div class="overflow-hidden rounded-lg border border-slate-200 bg-white">
        ...
        <div class="flex items-center gap-3 border-b border-slate-200 px-5 py-4">
            ...
            <div>
                ...
                <p class="text-[11px] text-slate-400">
                    Submitted, verified and rejected subscription payments
                </p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p id="paymentPeriodText" class="text-xs text-slate-400">
        ...
        <form id="paymentForm">
            ...
            <div class="space-y-4 p-5">
                ...
                <div class="rounded-lg bg-gradient-to-br from-indigo-600 to-violet-600 p-4 text-white">
                    <div class="flex items-center justify-between gap-3">
                        <div>
                            <p class="text-[10px] uppercase tracking-wide text-indigo-100">
                                Outstanding Amount
                            </p>

                            <p class="mt-1 text-xs text-indigo-100">
                                Maximum amount payable now
                            </p>
                        </div>

                        <span id="modalOutstanding" class="text-xl font-bold">
                            {{ $currency }}0.00
                        </span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-sm font-medium text-slate-400">
                            {{ $currency }}
                        </span>
...
async function loadSubscriptions(){
    ...
    try{
        ...
        document.getElementById('paymentHistory').innerHTML=`
            <tr>
                <td colspan="7" class="px-4 py-12 text-center text-xs text-red-500">
...
function renderCurrentDue(){
    ...
    const current=dues.find(
        due=>Number(due.year)===currentYear&&Number(due.month)===currentMonth
    );
    ...
    if(!current){
        ...
        document.getElementById('currentStatus').className=
            'w-fit rounded-md bg-slate-100 px-2.5 py-1 text-[10px] font-semibold text-slate-600';
    ...
    const currentOutstanding=outstanding(current);
...
function renderHistory(){
    ...
    tbody.innerHTML=dues.map(due=>`
        <tr class="transition hover:bg-slate-50">
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-xs text-slate-600">
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-xs font-semibold text-amber-600">
            ...
            <td class="whitespace-nowrap px-4 py-3 text-right text-xs font-semibold text-emerald-600">
                ${money(due.paid_amount)}
            </td>

            <td class="whitespace-nowrap px-4 py-3 text-right text-xs font-semibold ${
                outstanding(due)>0?'text-red-600':'text-emerald-600'
            }">
    ...
    tbody.innerHTML=payments.map(payment=>`
        <tr class="transition hover:bg-slate-50">
            ...
            <td class="max-w-[180px] px-4 py-3">
                <p class="truncate text-xs text-slate-500" title="${escapeHtml(payment.transaction_reference??'')}">
...
function statusBadge(status){
    ...
    return`
        <span class="inline-flex rounded-md border px-2 py-1 text-[10px] font-semibold ${classes[status]??classes.unpaid}">
...
function setStatusBadge(element,status){
    ...
    element.className=
        `w-fit rounded-md px-2.5 py-1 text-[10px] font-semibold ${
            classes[status]??
            classes.unpaid
        }`;
}

...
<body class="min-h-screen bg-slate-100 text-slate-900 antialiased">
    @php
        $authUser = auth()->user();
        $can = static fn(string $permission): bool => $authUser
            ? $authUser->hasAnyPermission(array_filter(explode(',', $permission)))
            : false;
        ...
        $linkClass = 'menu-link group flex items-center gap-2.5 rounded-md px-2.5 py-1 text-[12.5px] font-medium transition-all';
        ...
        $financeItems = [
            ...
            ['admin.finance.subscription-payments', 'admin.finance.subscription-payments*', 'bi-cash-stack', 'Subscription Payments'],
        ...
        $unreadContactMessagesCount = $canViewContactMessages ? \App\Models\ContactMessage::unread()->count() : 0;
    @endphp

    <div id="app" class="flex h-screen overflow-hidden">

        <div id="sidebarOverlay" class="fixed inset-0 z-40 hidden bg-slate-950/60 backdrop-blur-sm lg:hidden" onclick="closeSidebar()"></div>

        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 flex w-[250px] -translate-x-full flex-col bg-gradient-to-b from-slate-950 via-[#0d3b66] to-[#0a2f52] shadow-2xl transition-transform duration-300 ease-in-out lg:static lg:translate-x-0">
                ...
                @if ($membershipItems->isNotEmpty())
                    ...
                    <nav class="space-y-0.5">
                        @foreach ($membershipItems as [$permission, $route, $match, $icon, $label])
                            ...
                            <a href="{{ route($route) }}"
                                class="{{ $linkClass }} {{ $active ? $activeClass : $inactiveClass }}">
                                <span class="{{ $iconClass }}"><i
                                        class="bi {{ $icon }} text-[14px]"></i></span>
                                <span>{{ $label }}</span>
                            </a>
                        @endforeach
                    </nav>
                        ...
                        @if ($can('Finance.view'))
                            <div>
                                <button type="button" onclick="toggleFinanceMenu()"
                                    ...
                                    <span class="{{ $iconClass }}"><i
                                            class="bi bi-cash-coin text-[14px]"></i></span>
            ...
            <div class="shrink-0 border-t border-white/[0.08] p-3">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="truncate text-[12px] font-semibold text-white">{{ $authUser?->name ?? 'User' }}
                        </div>
                    ...
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            @if ($can('Approval.view'))
                                <a href="{{ route('admin.approvals') }}" target="_blank" rel="noopener noreferrer"
                                    class="hidden shrink-0 items-center gap-1.5 rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-[11px] font-semibold text-emerald-700 transition hover:border-emerald-300 hover:bg-emerald-100 sm:inline-flex">
                    ...
                    <div class="relative flex shrink-0 items-center gap-3 sm:gap-4">
                        ...
                        <button type="button" onclick="toggleUserMenu(event)"
                            ...
                            <div class="hidden text-left sm:block">
                                ...
                                <div class="max-w-[130px] truncate text-[10px] text-slate-400">
                                    {{ $authUser?->email ?? '' }}</div>
                            </div>
                        ...
                        <div id="userDropdown"
                            ...
                            <div class="border-b border-slate-100 px-4 py-3">
                                ...
                                <div class="mt-1 truncate text-[10px] font-medium text-sky-600">{{ $roleNames }}
                                </div>
                            </div>

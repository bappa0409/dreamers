...
#memberSidebar nav::-webkit-scrollbar-track{background:transparent}
#memberSidebar nav::-webkit-scrollbar-thumb{background:rgba(148,163,184,.35);border-radius:999px}
#memberSidebar nav::-webkit-scrollbar-thumb:hover{background:rgba(100,116,139,.55)}
#memberSidebar nav{scrollbar-width:thin;scrollbar-color:rgba(148,163,184,.35) transparent}
</style>
@stack('styles')
</head>
...
<div id="sidebarOverlay" onclick="closeSidebar()" class="fixed inset-0 z-40 hidden bg-slate-900/40 backdrop-blur-[1px] lg:hidden"></div>

<aside id="memberSidebar" class="fixed inset-y-0 left-0 z-50 flex w-[250px] -translate-x-full flex-col border-r border-slate-200 bg-white transition-transform duration-200 lg:static lg:z-auto lg:translate-x-0">
...
<button type="button" onclick="closeSidebar()" class="ml-auto flex h-8 w-8 items-center justify-center rounded-md text-slate-400 transition hover:bg-slate-100 hover:text-slate-600 lg:hidden">
...
<div>
<button type="button" onclick="toggleMemberMenu('subscriptionMenu','subscriptionChevron')" class="{{ $linkClass }} w-full {{ $subscriptionOpen?$linkActive:$linkInactive }}">
<i class="bi bi-credit-card-2-front {{ $iconClass }}"></i>
<span class="flex-1 text-left">Subscriptions</span>
<i id="subscriptionChevron" class="bi bi-chevron-down text-[9px] transition-transform duration-200 {{ $subscriptionOpen?'rotate-180':'' }}"></i>
</button>

<div id="subscriptionMenu" class="{{ $subscriptionOpen?'':'hidden' }} ml-[22px] border-l border-slate-200 pl-3">
<a href="{{ route('member.subscriptions') }}" class="flex items-center gap-2 rounded-md px-2 py-1 text-[11px] font-medium transition {{ request()->routeIs('member.subscriptions')?'bg-sky-50 text-sky-700':'text-slate-500 hover:bg-slate-50 hover:text-slate-700' }}">
<span class="h-1.5 w-1.5 rounded-full bg-current opacity-60"></span>
Monthly Dues
</a>

<a href="{{ route('member.subscription-payments') }}" class="flex items-center gap-2 rounded-md px-2 py-1 text-[11px] font-medium transition {{ request()->routeIs('member.subscription-payments')?'bg-sky-50 text-sky-700':'text-slate-500 hover:bg-slate-50 hover:text-slate-700' }}">
<span class="h-1.5 w-1.5 rounded-full bg-current opacity-60"></span>
Payment History
</a>
</div>
...
<a href="{{ route('member.shares') }}" class="{{ $linkClass }} {{ $active?$linkActive:$linkInactive }}">
...
<a href="{{ route('member.loans') }}" class="{{ $linkClass }} {{ $active?$linkActive:$linkInactive }}">
<i class="bi bi-bank {{ $iconClass }}"></i>
<span>Loans</span>
</a>

<div>
<button type="button" onclick="toggleMemberMenu('investmentMenu','investmentChevron')" class="{{ $linkClass }} w-full {{ $investmentOpen?$linkActive:$linkInactive }}">
<i class="bi bi-graph-up-arrow {{ $iconClass }}"></i>
<span class="flex-1 text-left">Investments</span>
<i id="investmentChevron" class="bi bi-chevron-down text-[9px] transition-transform duration-200 {{ $investmentOpen?'rotate-180':'' }}"></i>
</button>

<div id="investmentMenu" class="{{ $investmentOpen?'':'hidden' }} ml-[22px] border-l border-slate-200 pl-3">
<a href="{{ route('member.investments.active') }}" class="flex items-center gap-2 rounded-md px-2 py-1 text-[11px] font-medium transition {{ request()->routeIs('member.investments.active')?'bg-sky-50 text-sky-700':'text-slate-500 hover:bg-slate-50 hover:text-slate-700' }}">
<span class="h-1.5 w-1.5 rounded-full bg-current opacity-60"></span>
Active Investments
</a>

<a href="{{ route('member.investments.completed') }}" class="flex items-center gap-2 rounded-md px-2 py-1 text-[11px] font-medium transition {{ request()->routeIs('member.investments.completed')?'bg-sky-50 text-sky-700':'text-slate-500 hover:bg-slate-50 hover:text-slate-700' }}">
<span class="h-1.5 w-1.5 rounded-full bg-current opacity-60"></span>
...
<a href="{{ route('member.notifications') }}" class="{{ $linkClass }} justify-between {{ $active?$linkActive:$linkInactive }}">
<span class="flex items-center gap-2.5">
<i class="bi bi-bell-fill {{ $iconClass }}"></i>
<span>Notifications</span>
</span>

<span id="sidebarNotificationBadge" class="{{ $notificationUnreadCount>0?'':'hidden' }} min-w-[18px] rounded-full bg-red-500 px-1.5 py-0.5 text-center text-[9px] font-bold leading-none text-white">
...
<button type="button" onclick="openSidebar()" class="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50 lg:hidden" aria-label="Open menu">
...
<button type="button" onclick="toggleUserMenu(event)" class="group flex items-center gap-2 rounded-md py-1 pl-1 pr-2 transition hover:bg-slate-50">
...
<div id="notificationDropdown" class="absolute right-12 top-[48px] z-50 hidden w-[350px] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl shadow-slate-900/10">
...
<button id="notificationMarkAllButton" type="button" onclick="event.stopPropagation();markAllNotificationsRead()" {{ $notificationUnreadCount<=0?'disabled':'' }} class="text-[10px] font-semibold text-sky-600 transition hover:text-sky-700 disabled:cursor-not-allowed disabled:opacity-40">
Mark all read
</button>
</div>
...
<a href="{{ route('member.notifications') }}" class="flex h-9 w-full items-center justify-center gap-2 rounded-md text-[11px] font-semibold text-sky-600 transition hover:bg-sky-50 hover:text-sky-700">

...
class SubscriptionService
{
    ...
    public function generateDue(
        ...
        return DB::transaction(function () use ($subscription, $year, $month, $userId) {
            ...
            $journal = $this->accounting->post([
                'idempotency_key' => "subscription:due:{$due->id}",
    ...
    public function verifyPayment(
        ...
        return DB::transaction(function () use ($payment, $verifiedBy, $note) {
            ...
            $transaction = $this->accounting->post([
                'idempotency_key' => "subscription:payment:{$payment->id}",
    ...
    public function applyLateFine(
        ...
        return DB::transaction(function () use ($due, $userId) {
            ...
            $this->accounting->post([
                'idempotency_key' => "subscription:fine:{$due->id}",
    ...
    $plan=$due->subscription?->plan;

    if(!$plan){
        return 0;
    }

    $type=(string)$plan->fine_type;

    $value=max(
        round(
            (float)$plan->fine_value,
            2
        ),
        0
    );
    ...
    protected function generatePaymentNumber(): string
    {
        ...
        return $this->numberSequenceService->next(
            key: "subscription-payment:{$month}",
            prefix: $prefix,
            digits: 6,
